...
create or replace function public.issue_campaign_challenge(p_site text, p_nonce uuid, p_ip_hash text, p_expires_at timestamptz)
returns text language plpgsql security definer set search_path = public, pg_temp as $$
declare current_count integer;
begin
  if p_site not in ('luke', 'annalynn') or char_length(p_ip_hash) not between 40 and 64
     or p_expires_at <= now() or p_expires_at > now() + interval '6 minutes' then return 'invalid'; end if;
...
create or replace function public.consume_campaign_submission(p_site text, p_nonce uuid, p_ip_hash text)
returns text language plpgsql security definer set search_path = public, pg_temp as $$
declare accepted_nonce uuid; current_count integer;
begin
  if p_site not in ('luke', 'annalynn') or char_length(p_ip_hash) not between 40 and 64 then return 'invalid'; end if;
  update public.campaign_captcha_nonces set used_at = now()
