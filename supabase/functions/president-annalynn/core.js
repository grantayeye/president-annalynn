export const CAMPAIGN = "annalynn";
export const MAX_BODY_BYTES = 8192;
export const CHALLENGE_MIN_AGE_MS = 2500;
export const CHALLENGE_MAX_AGE_MS = 5 * 60 * 1000;

const allowedFields = new Set(["name", "email", "idea", "website", "privacyConsent", "captchaAnswer", "captchaToken"]);
...
export function validateSubmission(input) {
  ...
  if (keys.length !== allowedFields.size || keys.some((key) => !allowedFields.has(key))) return { ok: false, code: "INVALID_REQUEST" };
...
export async function createChallenge(secret, campaign = CAMPAIGN, now = Date.now(), random = crypto.getRandomValues.bind(crypto)) {
  ...
  const payload = { v: 1, campaign, nonce: crypto.randomUUID(), a, b, issuedAt: now, expiresAt: now + CHALLENGE_MAX_AGE_MS };
...
export async function verifyChallenge(token, answer, secret, campaign = CAMPAIGN, now = Date.now()) {
  try {
    ...
    if (
      payload?.v !== 1 || payload.campaign !== campaign || typeof payload.nonce !== "string" || !/^[0-9a-f-]{36}$/.test(payload.nonce) ||
      !Number.isInteger(payload.a) || payload.a < 2 || payload.a > 9 || !Number.isInteger(payload.b) || payload.b < 2 || payload.b > 9 ||
      ...
      payload.expiresAt - payload.issuedAt !== CHALLENGE_MAX_AGE_MS || Number(answer) !== payload.a + payload.b
    ) return null;
    return payload;
  } catch { return null; }
}
...
export function buildEmail(data) {
  ...
  return {
    subject: "A new idea for Annalynn's campaign",
