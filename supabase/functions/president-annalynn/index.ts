import {
  buildEmail, CAMPAIGN, corsHeaders, createChallenge, MAX_BODY_BYTES, originIsAllowed,
  parseAllowedOrigins, rateLimitHash, validateSubmission, verifyChallenge
} from "./core.js";

const allowedOrigins = parseAllowedOrigins(Deno.env.get("ANNALYNN_ALLOWED_ORIGINS"));
...
const recipientEmail = requireEmailSecret("ANNALYNN_RECIPIENT_EMAIL");
const fromEmail = requireEmailSecret("ANNALYNN_FROM_EMAIL");
...
async function handleChallenge(request: Request, origin: string): Promise<Response> {
  ...
  const issued = await rpc("issue_campaign_challenge", { p_site: CAMPAIGN, p_nonce: challenge.payload.nonce, p_ip_hash: ipHash, p_expires_at: new Date(challenge.payload.expiresAt).toISOString() });
  if (issued !== "issued") {
...
async function handleSubmission(request: Request, origin: string): Promise<Response> {
  if (request.headers.get("content-type")?.split(";", 1)[0].trim().toLowerCase() !== "application/json") return response(origin, 415, { ok: false, code: "INVALID_REQUEST" });
  let input: unknown;
  ...
  const ipHash = await rateLimitHash(requestIp(request), captchaSecret);
  const consumed = await rpc("consume_campaign_submission", { p_site: CAMPAIGN, p_nonce: challenge.nonce, p_ip_hash: ipHash });
...
Deno.serve(async (request: Request) => {
  ...
  if (!originIsAllowed(origin, allowedOrigins)) {
    return new Response(JSON.stringify({ ok: false, code: "ORIGIN_NOT_ALLOWED" }), { status: 403, headers: { "Cache-Control": "no-store", "Content-Type": "application/json; charset=utf-8", "X-Content-Type-Options": "nosniff" } });
  }
