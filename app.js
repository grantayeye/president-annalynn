...
async function loadCaptcha(showFailureStatus = true) {
  ...
  try {
    ...
    if (typeof data.challenge !== "string" || typeof data.token !== "string") throw new Error("Challenge response was invalid");
...
idea.addEventListener("input", () => { ideaCount.textContent = `${idea.value.length} / 1500`; });
