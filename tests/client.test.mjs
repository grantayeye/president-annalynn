...
test("normalizes the submitted fields", () => {
  assert.deepEqual(normalizeFormValues({ name: "  Sam   Student ", email: " SAM@EXAMPLE.COM ", idea: "  Add a   spirit day.  ", website: " ", privacyConsent: true, captchaAnswer: " 7 " }), {
    name: "Sam Student", email: "sam@example.com", idea: "Add a spirit day.", website: "", privacyConsent: true, captchaAnswer: "7"
  });
});
...
test("page exposes accessible required controls and no recipient address", async () => {
  ...
  for (const id of ["name", "email", "idea", "privacy-consent", "captcha-answer", "form-status"]) assert.match(html, new RegExp(`id="${id}"`));
