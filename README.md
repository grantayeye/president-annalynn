...
A responsive campaign site for Annalynn's ninth-grade student council race. The public site is static and sends ideas through a campaign-specific Supabase Edge Function. Submissions are emailed through Resend and are not stored by the application.
...
The form intentionally stays unavailable until `config.js` points to a deployed function. For a complete local integration test, serve the site from an exact origin included in `ANNALYNN_ALLOWED_ORIGINS` and use a local or test Supabase function endpoint.
...
supabase db push
supabase functions deploy president-annalynn --no-verify-jwt
```
...
- `ANNALYNN_RECIPIENT_EMAIL`: Annalynn's private destination address.
- `ANNALYNN_FROM_EMAIL`: A verified Resend sender, such as `Campaign Ideas <ideas@presidentannalynn.com>`.
- `ANNALYNN_ALLOWED_ORIGINS`: An exact comma-separated list, such as `https://presidentannalynn.com,https://www.presidentannalynn.com,https://GITHUB_USER.github.io,http://localhost:8080`. Replace `GITHUB_USER` with the real Pages preview host and remove localhost after QA.
...
- Submit one idea from the apex and `www`, then confirm it reaches only Annalynn's inbox.
